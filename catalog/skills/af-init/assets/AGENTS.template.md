...
- APPLY "STOP-ANALYSIS" RULE: IF YOU ATTEMPT TO FIX AN ISSUE 2+ TIMES WITHOUT SUCCESS, STOP. DO NOT MAKE A 3RD ATTEMPT. INSTEAD, OUTPUT A "STOP-ANALYSIS REPORT" LISTING: 1) CURRENT STATE, 2) EXPECTED STATE, 3) DIFFERENCE, 4) HYPOTHESES. WAIT FOR USER CONFIRMATION.
- APPLY "DATA-FIRST" RULE: WHEN WORKING WITH EXTERNAL PROCESSES OR APIS, ALWAYS LOG AND INSPECT THE RAW DATA (JSON, STDOUT, ETC.) BEFORE WRITING PROCESSING LOGIC. DO NOT RELY ON ASSUMPTIONS ABOUT DATA STRUCTURE.
- APPLY "ARCHITECTURAL VALIDATION" RULE: BEFORE CHANGING CORE LOGIC OR INTERACTIONS, VISUALIZE THE SEQUENCE OF EVENTS (WHO CALLS WHOM, WHO WAITS FOR WHOM). IF THE LOGIC IS COMPLEX, CREATE A SEQUENCE DIAGRAM OR PSEUDOCODE IN `whiteboard.md`.
