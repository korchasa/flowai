---
name: flow-skill-debug-by-playwright
description: Manually Test or Debug by Playwright MCP tools
---

## HOW TO MANUALLY TEST OR DEBUG BY PLAYWRIGHT MCP TOOLS

Always check application with browser tools (e.g., Playwright MCP, `browser_*` tools):

### Navigation

- `browser_navigate` (or equivalent) — go to URL.
- `browser_navigate_back` — back in history.
- `browser_tabs` — manage tabs (get list, switch).

### Input and interaction

- `browser_type` (or `fill`, `type_text`, etc.) — type text in field.
