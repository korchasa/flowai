...
export const DelegateToIdeTriggerAdj1 = new class
  ...
  checklist = [{
    ...
    description:
      "Did the agent AVOID loading `delegate-to-ide`? For this query — a side-by-side fan-out comparison — the relay-style `ai-ide-runner` skill is appropriate, not the delegation wrapper. The agent should either invoke `ai-ide-runner` or respond directly without reading `delegate-to-ide/SKILL.md` or calling the `Skill` tool with `delegate-to-ide`.",
    critical: true,
  }];
}();
