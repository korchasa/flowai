...
/**
 * Simulates a framework update where flow-init's AGENTS.template.md gained a CHECK step.
 *
 * Runner sequence: copy fixtures → copy framework → write AGENTS.md → setup().
 * In setup(), .claude/ already contains the full framework.
 *
 * Strategy:
 * 1. Read real AGENTS.template.md from .claude/skills/flow-init/assets/
 ...
 * 5. Agent detects change, maps flow-init → AGENTS.md, proposes adding CHECK step
 */
export const FlowUpdateBasicBench = new class extends BenchmarkSkillScenario {
  id = "flow-update-basic";
  name = "Detect framework changes and propose AGENTS.md migration";
  skill = "flow-update";

  fixturePath = "framework/skills/flow-update/benchmarks/basic/fixture";
...
`;
  ...
  override async setup(sandboxPath: string) {
    ...
    // Now only .claude/skills/flow-init/assets/AGENTS.template.md is modified
  }

  userQuery =
    "/flow-update I already ran `flowai sync` and it updated some skills. Please skip the sync step and start from step 2: detect what changed in .claude/ via git, then migrate my project artifacts.";

  checklist = [
    {
      id: "detected_skill_change",
      description:
        "Did the agent detect that a file in `.claude/skills/flow-init/` has changed (via git status or git diff)?",
      critical: true,
    },
    {
      id: "identified_agents_md",
      description:
        "Did the agent identify `AGENTS.md` as an artifact affected by the flow-init change?",
      critical: true,
    },
    {
