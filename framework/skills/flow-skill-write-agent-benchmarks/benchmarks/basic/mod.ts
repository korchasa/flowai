...
export const WriteAgentBenchmarksBasicBench = new class
  ...
  id = "flow-skill-write-agent-benchmarks-basic";
  name = "Create Benchmark Scenario for a Skill";
  skill = "flow-skill-write-agent-benchmarks";
  stepTimeoutMs = 180_000;

  userQuery =
    '/flow-skill-write-agent-benchmarks Create a benchmark scenario for the skill "flow-skill-example" that tests whether the agent can rename a variable across all files in a project. The skill is at framework/skills/flow-skill-example/SKILL.md.';

  override setup(sandboxDir: string): Promise<void> {
    // Create a minimal skill SKILL.md so the agent has something to reference
    Deno.mkdirSync(`${sandboxDir}/framework/skills/flow-skill-example`, {
      recursive: true,
    });
    Deno.writeTextFileSync(
      `${sandboxDir}/framework/skills/flow-skill-example/SKILL.md`,
      [
        "---",
  ...
  checklist = [
    {
      ...
      description:
        "Did the agent create a benchmark scenario file at 'framework/skills/flow-skill-example/benchmarks/*/mod.ts'?",
      critical: true,
    },
    {
