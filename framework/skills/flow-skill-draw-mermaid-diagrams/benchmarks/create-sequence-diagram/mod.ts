...
export const DrawMermaidSequenceBench = new class
  ...
  id = "flow-skill-draw-mermaid-diagrams-sequence";
  name = "Create a Mermaid sequence diagram for an API auth flow";
  skill = "flow-skill-draw-mermaid-diagrams";

  userQuery =
    "/flow-skill-draw-mermaid-diagrams Create a sequence diagram showing an OAuth2 authorization code flow between a User, Browser, Auth Server, and Resource Server. Include the redirect, token exchange, and API call with the access token. Save it to docs/auth-flow.md.";
