...
export const TsDenoStyleTriggerPos1 = new class extends AcceptanceTestScenario {
  id = "flowai-setup-agent-code-style-ts-deno-trigger-pos-1";
  ...
  checklist = [{
    id: "skill_invoked",
    description:
      "Did the agent load and act on `flowai-setup-agent-code-style-ts-deno` in response to this query? Look in the trace for a `Skill` tool call or a read of the skill's `SKILL.md` for `flowai-setup-agent-code-style-ts-deno`.",
    critical: true,
  }];
}();
