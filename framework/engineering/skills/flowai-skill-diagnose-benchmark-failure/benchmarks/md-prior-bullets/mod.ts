 ...
 *   - read benchmarks/runs/latest/<scenario-id>/run-1/judge-evidence.md
...
export const DiagnoseBenchMdPriorBulletsBench = new class
  ...
  override async setup(sandboxDir: string): Promise<void> {
    ...
    //   benchmarks/runs/latest/<scenario-id>/run-1/{judge-evidence.md, sandbox/.claude/skills/...}
  ...
  checklist = [
    {
      ...
      description:
        "Did the agent read the judge-evidence.md file from the failed run dir (benchmarks/runs/latest/<scenario-id>/run-1/judge-evidence.md)? Look for a Read tool call (or bash cat) targeting that path.",
      critical: true,
    },
    {
      id: "read_sandbox_skill",
      description:
        "Did the agent read the sandbox SKILL.md the failing agent saw, at benchmarks/runs/latest/<scenario-id>/run-1/sandbox/.claude/skills/flowai-skill-conduct-qa-session/SKILL.md? Look for a Read tool call targeting that path.",
      critical: true,
    },
    {
