...
3. **Gather Context**
   ...
   **Parallel Delegation** (after gathering context):
   ...
   - **Collect what you dispatched**: on many hosts the dispatch call returns only a launch handle (an agent id plus usage counters) and the work continues in the background, so the result arrives later through a separate wait/collect call or a completion notification. Give that call a budget in minutes, not seconds, and re-issue it until the task reports finished. A collection call that times out means "not finished yet", not "delegation unsupported" — re-issue it before concluding anything. Never write a delegated finding you did not actually receive.
