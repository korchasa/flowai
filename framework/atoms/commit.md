...
6. **Session Complexity Check → Auto-Invoke Reflect**
   ...
   - If **any** of these signals are detected:
     ...
     d. Invoke the `reflect` skill directly (via the Skill tool, native slash-command execution, or inline execution of its `SKILL.md` instructions — whichever the host IDE supports). **Carry the decision from (c) AND the return obligation into the invocation itself** — append this text verbatim to the context you pass reflect, substituting the user's real reply from (c) — if you are about to write `apply` without having read it in a message from the user, you skipped (c): go back and ask. `The user has already decided about your proposed edits: <apply | report only>. Do NOT ask again. On "apply", apply them yourself; then control returns to the calling workflow at its post-reflect step: run git status and commit everything you changed as a separate agent: commit. Do not close out the session.` By the time reflect ends you will be many steps away from this file, and the only instruction still in front of you is the one you carried in.
