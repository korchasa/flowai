...
6. **Session Complexity Check → Auto-Invoke Reflect**
   ...
   - If **any** of these signals are detected:
     ...
     b. **Pre-command signal check**: if the signals appear only in the invocation message (i.e., the problematic interactions predated this command and are not visible in the conversation history), output: "You mentioned a rough session — briefly describe what went wrong and what you corrected. This will be included as reflect context." **Then go straight on to (c) in the same message — nothing waits on an answer to THIS question.** You are asking for detail you would like, not for input you need: the invocation message already carries the signals that fired the check, and that is context enough to run reflect on. Waiting for an answer that may never come turns the whole complexity check into a dead end — the announcement in (a) is made, reflect never runs, and steps 7 and 8 are never reached. If the answer does arrive, fold it into the reflect context you are already building.
     c. **Approval gate — ask it HERE, before reflect runs, and nowhere else in this branch.** One line: `reflect may propose edits to your instruction files (AGENTS.md, CLAUDE.md). Reply "apply" and I will apply the approved edits and commit them; reply "report only" for findings without changes.` Put it in the SAME message as (b) when (b) fired, and wait for this answer — it is the one question in the reflect branch that does pause. Its position is the whole point: an approval asked at the END, after reflect's report is written, arrives when nothing is left to do, so the turn ends on the user's reply and the applied edit is never committed. Asked here, the reply is followed by the entire reflect run, the edit, the cleanup commit and the clean-state check, so answering and finishing can no longer be the same moment. **Asking is not enough — end your turn on the question and write nothing after it until the reply arrives.** A question followed in the same breath by "now applying the edits" is not a gate, it is an announcement; the user never got to answer, and the safeguard existed only as text. This is the one place in the whole workflow where stopping is correct, and it is the opposite of everything else here: elsewhere you are told never to stop mid-run, so the habit works against you exactly here. **You may not skip this question and you may not answer it on the user's behalf.** There is no default and no pre-authorization to inherit: invoking this workflow authorises the commits it makes, never edits to the user's own instruction files. Until the question has gone out in a message and a reply has come back, you hold no decision — "the user pre-authorized apply" written by you, about a question you never asked, is the one way this gate fails.
     d. Invoke the `reflect` skill directly (via the Skill tool, native slash-command execution, or inline execution of its `SKILL.md` instructions — whichever the host IDE supports). **Carry the decision from (c) AND the return obligation into the invocation itself** — append this text verbatim to the context you pass reflect, substituting the user's real reply from (c) — if you are about to write `apply` without having read it in a message from the user, you skipped (c): go back and ask. `The user has already decided about your proposed edits: <apply | report only>. Do NOT ask again. On "apply", apply them and then commit them yourself, before you finish: stage only the files you edited, by explicit path, and make ONE separate commit of type agent: — do not amend, and do not invoke any commit workflow for it. Then report what you committed.` Committing is asked of reflect inside its own run on purpose: what a sub-run is told to do after it ends does not happen, and three rounds of instructing it to hand control back proved exactly that.
     e. Do NOT ask whether to RUN reflect — that is not the user's call here, and neither (b) nor (c) is such a request: (b) gathers missing information, (c) decides what happens to reflect's proposed edits.
     f. **`reflect` does not end this workflow — say so to yourself before you hand control over.** The moment it returns you are back here, at step 7, with two steps still owed. This is where the workflow is most often abandoned: reflect closes with a report and, if you approved one, an applied edit, and that ending reads like the ending. It is not yours. Reflect's report is not your final report, and "applied the corrective action" is not a place to stop — the edit it just made is precisely what step 7 has to commit.
...
7. **Post-Reflect Cleanup Commit** _(skip if reflect produced no edits)_
   - **This step is now a check, and its fallback.** Under step 6d reflect commits its own edits inside its own run, so the usual outcome here is a clean tree and nothing to do. Verify it rather than assume it — and if reflect left edits uncommitted, they are yours to commit right here, in this same turn.
   - **Announcing this step is not performing it, and reflect's ending does not end your turn.** "Control returns to step 7" is the most common last sentence this workflow ever produces: it reads like a handover, so the turn stops there and the edit is never committed. There is nobody to hand over to — you are the one who runs step 7, and you run it now, without waiting for the user to say anything.
