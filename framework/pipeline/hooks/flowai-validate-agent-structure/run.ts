...
/**
 * flowai-validate-agent-structure hook: validates that pipeline pack agents
...
export function isPipelineAgent(filePath: string): boolean {
  ...
  return /framework\/pipeline\/agents\/flowai-agent-.*\.md$/.test(filePath) ||
    /\.claude\/agents\/flowai-agent-.*\.md$/.test(filePath);
}
...
if (import.meta.main) {
  ...
  if (!result.valid) {
    console.log(JSON.stringify({
      additionalContext:
        `[flowai-validate-agent-structure] Agent ${filePath} is missing required sections: ${
          result.missing.join(", ")
