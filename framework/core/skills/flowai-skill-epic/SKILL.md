...
description: Use when the user asks to plan a LARGE feature that spans multiple sessions or phases — produces documents/tasks/<YYYY>/<MM>/epic-<name>.md with dependency-ordered phases, atomic tasks, and per-phase status tracking. Use flowai-skill-plan for single-session tasks. Do NOT trigger on generic planning, roadmap, or brainstorming requests.
