import { BenchmarkSkillScenario } from "@bench/types.ts";

// Adjacent skill: flowai-skill-jit-review (synthesize ephemeral tests to catch
// hidden regressions in a diff — narrower scope than full QA review).
export const ReviewTriggerAdj1 = new class extends BenchmarkSkillScenario {
  id = "flowai-skill-review-trigger-adj-1";
  name = "JIT regression test scan (adjacent)";
  skill = "flowai-skill-review";
  agentsTemplateVars = { PROJECT_NAME: "Sandbox" };
  userQuery =
    "Tests pass but I'm worried I broke something subtle. Synthesize JIT tests against my staged diff to catch hidden regressions.";
  checklist = [{
    id: "skill_not_invoked",
    description:
      "Did the agent AVOID loading `flowai-skill-review`? For this query the skill is not appropriate; the agent should either invoke a different skill or respond directly without reading `flowai-skill-review/SKILL.md` or calling the `Skill` tool with `flowai-skill-review`.",
    critical: true,
  }];
}();
