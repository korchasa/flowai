...
// Verifies FR-DOC-RESCUE: flowai-skill-reflect detects decision passages in
...
export const ReflectRescuesDecisionAsTaskBench = new class
  ...
  id = "flowai-skill-reflect-rescues-decision-as-task";
  name =
  ...
  checklist = [
    ...
    },
    {
      ...
      description:
        "Did the agent NOT recommend the deprecated '/flowai-skill-plan-adr' command on its '**Recommended action:**' line? The rescue contract now points to '/flowai-plan-exp-permanent-tasks' exclusively. Pass if the only Recommended action line(s) in the rescue block name '/flowai-plan-exp-permanent-tasks' (the agent may MENTION 'flowai-skill-plan-adr' elsewhere as historical context, e.g. quoting old AGENTS.md text — that is allowed; the failing condition is recommending it as the action to invoke).",
      critical: true,
    },
    {
