...
When the root cause is outside your control (missing API keys/URLs, missing generator scripts, unavailable external services, wrong environment configuration) → STOP immediately and ask the user for the correct values. Do not guess, do not invent replacements, do not create workarounds. A default value is an invented replacement, and so is a configurable alternative: if you would have to choose what the fallback should be, you are guessing. Restructuring the code so the missing input is not needed yet is a workaround, not a fix. **The test is authorization, not access.** Being able to produce the missing thing yourself is not permission to add it: if the generator script is absent, write no generator, and that it would fetch real data from a public source changes nothing — the question is whether this artefact should exist and in what shape, and only the user answers that. `Proactive Resolution` does not reach here either; there is no fact to look up.
