...
- **A missing input is a blocker, not a gap to fill.** When the code needs something that is not there — a module an import names, a generator script, an API key, a fixture, a dataset — stop and ask for it. Do NOT reconstruct it from what you can infer, and do NOT write it from what you happen to know. Accuracy is not the test: values you believe are real still arrive with no provenance and no regeneration date, the hand-written file silently becomes the source of truth for everything downstream, and shaping it to look generated hides that instead of fixing it. "Fix the failing test" is not authorization to supply the missing input, and a comment in the code saying the artefact is generated — or must not be written by hand — is a decision already made, not an obstacle to route around. This binds the moment you notice the thing is missing, before you weigh whether you could produce a plausible substitute.
