...
4. **Protected-branch divergence gate**: if the remote branch is `main` / `master` AND the remote has commits the local does not (`git rev-list HEAD..@{u}` is non-empty), ASK the user before any push attempt. A force-with-lease against `main` with remote-ahead is the canonical regression — refuse without explicit per-push authorization.
...
3. **Safety Checks**
   - **Protected-branch divergence**: if the upstream is `origin/main` or `origin/master`, run `git fetch origin <CURRENT>` then `git rev-list --left-right --count HEAD...@{u}`. If the right count (commits the remote has and local does not) is > 0, ASK the user: "Remote `<CURRENT>` is ahead by N commit(s) the local branch does not have. Force-push would destroy them. Pull and rebase first, or abort?". Wait for an answer. Do NOT propose `--force-with-lease` as the default action.
