...
/**
 * Tests that adapt adapts agents to project specifics.
 *
 * Scenario:
 * 1. Python project with pytest/ruff stack (visible in AGENTS.md)
 * 2. diff-specialist agent installed with generic Deno references
 * 3. User runs /adapt --agents
 * 4. Agent should detect generic references, adapt via agent-adapter subagent
 *
...
export const FlowAdaptAgentsBasicBench = new class
  ...
  override sandboxState = {
    ...
    expectedOutcome:
      "Agent adapts console-expert agent: replaces generic Deno references with Python-specific ones, shows diff, asks confirmation",
  };
...
`;
    await Deno.writeTextFile(
      join(agentsDir, "console-expert.md"),
      genericAgent,
    );
  ...
  checklist = [
    ...
    },
    {
      ...
      description:
        "Did the agent launch a agent-adapter subagent (or similar adaptation agent) for the agent file?",
      critical: true,
    },
    {
      id: "showed_diff",
