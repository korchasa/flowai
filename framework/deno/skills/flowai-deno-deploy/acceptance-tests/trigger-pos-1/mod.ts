...
export const DenoDeployTriggerPos1 = new class extends AcceptanceTestScenario {
  id = "flowai-deno-deploy-trigger-pos-1";
  ...
  checklist = [{
    id: "skill_invoked",
    description:
      "Did the agent load and act on `flowai-deno-deploy` in response to this query? Look in the trace for a `Skill` tool call or a read of the skill's `SKILL.md` for `flowai-deno-deploy`.",
    critical: true,
  }];
}();
