...
export const WriteAgentBenchmarksTriggerPos1 = new class
  ...
  id = "flowai-write-agent-benchmarks-trigger-pos-1";
  ...
  checklist = [{
    id: "skill_invoked",
    description:
      "Did the agent load and act on `flowai-write-agent-benchmarks` in response to this query? Look in the trace for a `Skill` tool call or a read of the skill's `SKILL.md` for `flowai-write-agent-benchmarks`.",
    critical: true,
  }];
}();
