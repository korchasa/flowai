...
export const WriteAgentBenchmarksBasicBench = new class
  ...
  id = "flowai-write-agent-benchmarks-basic";
  ...
  userQuery =
    '/flowai-write-agent-benchmarks Create a benchmark scenario for the skill "flowai-example" that tests whether the agent can rename a variable across all files in a project. The skill is at framework/skills/flowai-example/SKILL.md.';

  override setup(sandboxDir: string): Promise<void> {
    // Create a minimal skill SKILL.md so the agent has something to reference
    Deno.mkdirSync(`${sandboxDir}/framework/skills/flowai-example`, {
      recursive: true,
    });
    Deno.writeTextFileSync(
      `${sandboxDir}/framework/skills/flowai-example/SKILL.md`,
      [
        "---",
        "name: flowai-example",
  ...
  checklist = [
    {
      ...
      description:
        "Did the agent create a benchmark scenario file at 'framework/skills/flowai-example/benchmarks/*/mod.ts'?",
      critical: true,
    },
    {
      id: "extends_base_class",
      description: "Does the scenario class extend AcceptanceTestScenario?",
      critical: true,
    },
    {
      id: "has_checklist",
