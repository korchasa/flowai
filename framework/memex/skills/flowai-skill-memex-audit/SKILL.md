...
Invoked when the user wants a memex health check (e.g., "audit my memex", "check the memex for dead links", `/flowai-skill-memex-audit` for audit-only or `/flowai-skill-memex-audit --fix` to apply trivial auto-fixes).
...
- **Coverage gaps**: read `index.md` and identify domains with only 1–2 pages. Suggest 3–5 questions the memex cannot yet answer well — candidates for `flowai-skill-memex-ask` or further `flowai-skill-memex-save`.
