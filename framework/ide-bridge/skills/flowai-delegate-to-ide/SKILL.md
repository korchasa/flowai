...
description: Delegate a task to another AI IDE's CLI (codex / claude / opencode / cursor-agent) through an isolated-context subagent. Triggers on "delegate to <ide>", "have <ide> do <task>", "execute <task> in <ide>", "offload to <ide>". For one-shot relay or fan-out comparison use `flowai-ai-ide-runner` instead.
---
...
## When to use this skill vs `flowai-ai-ide-runner`

- Use **this skill** (`flowai-delegate-to-ide`) when the user wants the other
...
- Use **`flowai-ai-ide-runner`** when the user wants a *one-shot relay*
...
- **Claude Code**: use the `Agent` / `Task` tool with
  `subagent_type=flowai-ide-bridge-worker`.
- **OpenCode**: use the `@flowai-ide-bridge-worker <task prompt>` mention
  syntax.
- **Cursor / Codex** (no native subagent dispatch): subagent invocation is
  ...
  and route them to `flowai-ai-ide-runner` for one-shot relay. Do NOT
...
- compare runs across IDEs or models (use `flowai-ai-ide-runner`);
- fan out one prompt across multiple IDEs (also `flowai-ai-ide-runner`);
