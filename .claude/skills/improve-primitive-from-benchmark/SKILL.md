...
- **Loop termination** (when this skill is invoked iteratively): STOP the loop and hand back to the user when (a) Phase 4 yields no fix candidate that survives the critic, or (b) Phase 3 surfaces no NEW failure mode that admits a GENERALIZABLE fix — i.e. every remaining failure is either already root-caused with its fix shipped or critic-rejected, or fixable only by a benchmark-shaped crutch that fails the Phase 4 anti-overfit gate (a mode whose only cure is overfitting is out of scope for this loop, not a reason to keep iterating; growing the pool is a human decision), or (c) the same atom would be edited a 3rd time across iterations — repeated re-editing means the diagnosis is unstable and needs human review. The aggregate resolved count is NEVER a termination or continuation criterion — it is noise-dominated at this pool size. Announce which condition fired.
