...
In this project, the hook source is at `framework/pipeline/hooks/flowai-validate-agent-structure/run.ts`.
...
In this project, the hook source is at `framework/pipeline/hooks/flowai-validate-artifact/run.ts`.
...
In this project: `framework/pipeline/agents/flowai-reflection-protocol.md`.
...
In this project, the reference implementation is at `framework/pipeline/skills/flowai-pipeline-github-sdlc/SKILL.md`,
with agents at `framework/pipeline/agents/flowai-agent-*.md`.

## Project-Specific Notes

- All pipeline primitives (skills, agents, hooks) MUST use the `flowai-` prefix per naming convention (NP-1).
- Hooks and validation scripts are written in TypeScript and run with Deno.
- Verify changes with `deno task check` (runs fmt, lint, and tests).
- New or changed skills/agents require benchmark TDD: write benchmark scenario first (RED), then update skill (GREEN).
- Benchmark scenarios live alongside their skill/agent: `framework/<pack>/skills/<skill>/benchmarks/<name>/mod.ts`.
