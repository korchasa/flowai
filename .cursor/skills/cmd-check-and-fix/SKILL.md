...
Iteratively run project verification tasks (primarily `deno task check`, but
also others if requested) and fix detected errors until the project is clean or
a limit is reached.
...
2. **Identify Verification Methods**
   ...
   - **Instruction-Defined**: Parse the current user prompt and task context for
     _additional_ required checks (e.g., "run tests for X", "check strict
     mode").
...
3. **Iterative Repair Loop (Max 5 attempts)**
   - **Step 3.1: Execute Checks**
     ...
     - For each command:
       - Run the command.
       - **Fail-Fast**: If _any_ command fails, stop the sequence immediately to
         address the error.
   ...
   - **Step 3.3: Apply Fixes**
     ...
     - Apply a fix.
     - _Self-Correction_: If a fix is complex, verify it locally by reading the
       file again.
...
5. **Report**
   ...
   - List any remaining unresolvable errors. </step_by_step>
