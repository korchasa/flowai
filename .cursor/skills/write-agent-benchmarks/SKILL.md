...
2.  **Create File**: Create a new file in `scenarios/` (e.g., `git-committer.bench.ts`).
...
export const scenario: BenchmarkScenario = {
  ...
  targetAgent: "git-committer", // ID of the agent to test
  ...
  userQuery: "Fix the bug in main.ts (change to 'fixed') and commit the change.",
