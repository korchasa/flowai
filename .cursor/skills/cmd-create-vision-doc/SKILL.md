...
## Overview
Interactive process to create or update the Product Vision document (`documents/vision.md`) by interviewing the user.
...
1. **Initialize**
   ...
   - Check if `documents/vision.md` exists and decide whether to update or rewrite.
2. **Interview Phase**
   - Conduct interview for each section: Vision Statement, Target Audience, Problem Statement, Solution & Differentiators, Business Goals, Strategy & Roadmap, Risks & Assumptions.
   - Save progress to `whiteboard.md`.
3. **Drafting Phase**
   - Synthesize answers into the vision template.
   - Present draft to user for review.
4. **Finalization**
   - Incorporate feedback and write to `documents/vision.md`.
</step_by_step>
