...
2. **Workspace sync**
   ...
   - Keep content compact (lists, tables, YAML, Mermaid diagrams) and use concise language.
   - Eliminate filler phrases, prioritize direct language, and maintain clear structure with headings/sections.
...
3. **Change review**
   ...
   - **Inclusion Policy**: Automatically include all verified non-PII changes. **DO NOT** ask the user for selection unless potential PII (secrets, tokens, personal data) is detected. If PII is found, stop and ask for confirmation.

4. **Commit creation**
   - Compose the message in **English** per Conventional Commits 1.0.0 (Strict Profile).
