...
- **The user's description of a defect is its specification.** When your own measurements suggest the defect has a narrower or different shape than the user stated, the measurement refines the FIX, never the definition of what is broken. Say what you measured and ask, instead of re-scoping silently. Observed 2026-09-17..19: the user reported that a variant was written as one paragraph; six sandboxed runs happened to fail only in the LATER option sets of a reply, and that coincidence was written into two acceptance checklists, a task file and two SRS lines as "the defect lives in secondary option sets". It took two corrections from the user — "the problem was that a variant was written as one paragraph" and "no, it was everywhere, not only in the second variant" — and a full rewrite of all four artefacts.
...
- `.claude/skills/`, `.claude/agents/`: Dev-only resources (not distributed). Framework commands + skills install into `.claude/skills/` (commands get `disable-model-invocation: true` injected by the plugin builder, `scripts/build-plugins.ts`).
...
- `.claude/skills/diagnose-acceptance-run/`: dev-only addendum to the shipped `diagnose-benchmark-failure` skill — the run-directory layout of this repo, the artefact file names, the jq and resume commands, and the taxonomy codes that point at our own runner. The shipped skill stays harness-agnostic; everything flowai-specific lives here.
...
- **Distribution** — the plugin marketplace [korchasa/flowai-plugins](https://github.com/korchasa/flowai-plugins), a generated repo: `deno task build-plugins` renders every pack into `dist/claude-plugins/`, `validate-plugins` checks it, and the CI release job pushes that tree downstream on each release (FR-PACKS). `deno task sync-plugins-local` points the local Claude Code marketplace at this checkout for dogfooding (README §Dogfooding the local plugin marketplace). Cursor and OpenCode have no plugin marketplace: their users run the build themselves and copy the rendered skills into `.claude/skills/`.
- `.github/workflows/ci.yml`: CI/CD for this (framework) repo — runs `deno task check` and, on `feat:`/`fix:` commits, cuts a `chore(release)` plus a `v<version>` GitHub release, then builds the plugins and pushes them to `korchasa/flowai-plugins`.

## Test Systems — two of them, never one word for both

This project runs two unrelated test systems. The bare word **benchmark** (or the user's «бенчмарки») means the SWE-rebench A/B and nothing else; the acceptance runner is always called **acceptance tests** / **acceptance scenarios**.

- **Acceptance tests** — behavioural scenarios for the framework's own primitives (commands, skills, agents). Source: `framework/<pack>/<kind>/<name>/acceptance-tests/<scenario>/mod.ts`. Each scenario runs the primitive in a sandboxed IDE session and an LLM judge scores a checklist. Run: `deno task acceptance-tests` (all), `deno task acceptance-tests -f <primitive-id>` (one primitive); the dev skill `acceptance-tests-all` adds a 5-subagent parallel mode. Artifacts: `acceptance-tests/runs/latest/<scenario-id>/run-N/` (judge-evidence.md, sandbox, bench-home), verdict cache in `acceptance-tests/cache/`. Governed by FR-ACCEPT*, SDS §3.4, and the "Acceptance Test TDD" section below. A full sweep is hours of LLM time — the user runs it unless they authorise it for the task.
- **SWE-rebench benchmark** — same-harness A/B of flowai vs bare codex over a frozen pool of SWE-rebench instances; the signal is the baseline-fail → flowai-pass cell. Run: `deno task benchmark <subcommand>` (`scripts/benchmark.ts`: setup, pool2-*, verify, retro, cells-*). Artifacts: result cells read by `scripts/project-status.py`, reports under `documents/benchmarks/`. Governed by FR-BENCH-SWE, SDS §3.22; the dev skill `improve-primitive-from-benchmark` is built on this run.
...
2. **IDE slash command**: a flat `.md` file under `.{ide}/commands/` (e.g. `.claude/commands/my-cmd.md`). Owned by the user; the framework never writes there. Framework commands never land in this directory.
...
- `framework/atoms/<name>.md` → SRS FR-SKILL-COMPOSE, [REF:sds:3-0 | SDS §3.0 Primitive Inventory]; atomic source consumed by the generator. Excluded from the distributed tree (see `scripts/check-pack-refs.ts --leakage`).
- `framework/composites/<name>.md` → SRS FR-SKILL-COMPOSE, [REF:sds:3-0 | SDS §3.0 Primitive Inventory]; composite wrapper consumed by the generator. Excluded from the distributed tree.
- `framework/composites.yaml` → SRS FR-SKILL-COMPOSE, SDS §3.1.1 Composite Skill Generation subsection, [REF:sds:3-0 | SDS §3.0 Primitive Inventory]. Excluded from the distributed tree.
...
- `scripts/build-plugins.ts`, `scripts/validate-plugins.ts`, `scripts/sync-plugins-local.ts` → [REF:fr:packs | FR-PACKS], [REF:fr:dist.marketplace | FR-DIST.MARKETPLACE], [REF:fr:dist.mapping | FR-DIST.MAPPING], [README §Installation](README.md#installation), README §Dogfooding the local plugin marketplace.
...
- `scripts/task-*.ts` → [README §Development Setup](README.md#development-setup), AGENTS.md §Standard Interface
- `documents/ides-difference.md` is READ-ONLY reference for the cross-IDE clauses (FR-HOOK-RESOURCES, FR-DIST.MAPPING, FR-IDE-BRIDGE-WORKER) — update only when IDE capabilities change.
...
- **DoD Evidence must be satisfiable, not only falsifiable**: when you WRITE an evidence command, run it twice — once against the current (undone) state, where it must fail, and once against the state the item describes as done, where it must pass. A command that cannot pass while the requirement holds is not a gate; it forces a false `[x]` or damage to the product. Two ways this happens, both observed on 2026-09-17: a blanket `! grep -rn '<name>'` also forbids the sentences that STATE a prohibition on that name, so the only way to go green is to delete the guard the requirement asked for; and an over-wide alternation (`! grep -E 'flowai-cli|benchmarks/'`) forbids a live directory the design still describes. Assert each clause of the item directly instead of mechanising it as the absence of one old name.
...
- **Variant Analysis**: When the path is non-obvious, propose variants with Pros/Cons/Risks per variant and trade-offs across them. Quality over quantity — one well-reasoned variant is fine if the path is clear. When changing a workflow primitive (skill, command, agent) that has existing acceptance-test coverage, ALWAYS surface ≥2 variants before editing — the "obvious path" heuristic does not apply, since primitives have multiple valid attachment points (start, mid, end, separate phase) and each has different regression risk.
- **Variant Presentation**: The variants ARE the labeled options of the single question that asks the user to choose — each option carries its own Pros/Cons/Risks/Best-for, and the cross-variant trade-offs follow the option list inside the same question. Do NOT write the variants out a second time as standalone blocks before the question; two copies drift and the reader reads both to answer once.
- **Variant Layout**: Every property gets its own labelled line — `**Pros:**`, `**Cons:**`, `**Risks:**`, `**Best for:**` as four separate bullets under the option title, then the trade-offs as their own block after the option list. An option written as one paragraph is a defect, and so is an option squeezed into a single line with its properties separated by commas, semicolons or dashes: the reader compares options property by property, and a missing property must be visible as a missing line. **This binds on EVERY option set in a reply, not only the first one.** A later choice — a failure policy, a key to count on, a rollout order — is an option set too and carries the same four labelled lines plus its own trade-offs; a correctly laid-out first question followed by bare one-line options below it fails this rule. **The layout must survive Markdown rendering.** A letter tag such as `A.` is not a Markdown list marker, so an option title written on the line right after the question, or right after the previous option's last bullet, is merged into that line as continuation text and vanishes from the rendered reply. Put each option title in its own bold paragraph (`**A. <title>**`) with a blank line before and after it, and write the four property bullets flush left, not indented under a numbered question. Observed 2026-09-23 with markdown-it: options B and C rendered as the tail of the previous option's last bullet, and option A as the tail of the question line.
- **Self-Contained Questions**: Every question to the user is answerable from itself and its options alone. State what is being decided and what the answer changes, inside the question. "Which of the above?", a bare "Your choice?", and "Which variant do you prefer?" with nothing restated are defects.
...
1. **RED**: Write acceptance scenario (`framework/<pack>/<kind>/<name>/acceptance-tests/<scenario>/mod.ts`) for new/changed behavior. Run the scenario — it MUST fail (proves the scenario tests something real).
2. **GREEN**: Update SKILL.md (`framework/<pack>/<kind>/<name>/SKILL.md`) until the scenario passes.
3. **REFACTOR**: Improve text or scenario clarity. No behavior change. Re-run the scenario.
...
1. **RED**: Write acceptance scenario (`framework/<pack>/agents/<agent-name>/acceptance-tests/<name>/mod.ts`) for new/changed agent behavior. Use `AcceptanceTestAgentScenario` base class (field `agent` instead of `skill`). Run the scenario — it MUST fail.
2. **GREEN**: Update agent (`framework/<pack>/agents/<agent-name>.md`) until the scenario passes.
3. **REFACTOR**: Improve agent prompt or scenario clarity. No behavior change. Re-run the scenario.
...
- EVERY command/skill/agent change MUST have a corresponding acceptance scenario (new or existing) that covers the changed behavior.
- Write the scenario BEFORE changing the primitive (RED phase). If the scenario already passes before the change, the scenario is not testing the right thing — revise it.
- Acceptance scenarios test OBSERVABLE BEHAVIOR (checklist items), not internal wording.
...
- **Run the scenario's verification command on the UNTOUCHED fixture before planting a defect.** A scenario that damages a fixture and asks the agent to run a check is only meaningful when that check is green beforehand. Fixtures are not green by default: `framework/core/acceptance-tests/agents-rules/fixture` fails `deno task check` out of the box, because `src/fetchers/local-fetcher/fetch-content.e2e.test.ts` reads a `fixtures/` directory that is not in the repo. On a red baseline the judge cannot separate your planted failure from the ambient one. Copy the fixture to the scratch dir, run the command there, and pick one that is green — `deno lint` instead of `deno task check`, if that is what it takes. The same run tells you which tests the fixture really has: `find -name '*_test.ts'` reports nothing on a fixture whose 77 tests are named `*.test.ts`, and that empty result reads as "no coverage" when it means "wrong glob". The same holds for files that `setup()` writes into the sandbox: materialize them onto the fixture copy and run the check there BEFORE the first scenario run. A `jsr:` import in a setup-written test fails the sandbox's `deno lint` (`no-import-prefix`), and the review then stops with Request Changes on the fixture rather than on the primitive — observed 2026-09-03 on `review-commit-push-full-cycle-success`, one full scenario run spent on it. Assert without imports in fixture tests.
- **A rule about how the agent WRITES needs a defect that reproduces on the arm the runner drives.** Layout, phrasing and formatting rules are the case: the runner's default arm is codex `gpt-5.6-terra`, and it lays options out as labelled lists whatever the rule text says. Both scenarios written for the variant-layout rule on 2026-09-19 PASSED on the pre-fix text — `plan-variant-properties-labelled` 4/4 in 152 s, `agents-rules-variant-analysis` 4/4 in 57 s — so neither could ever go red and neither is that rule's acceptance. Before authoring such a scenario, run its query against HEAD on the default arm and confirm the defect appears; a scenario that is green on both texts is a regression guard at best, and the SRS must say so instead of citing it as acceptance. When the defect is a Claude one, the run needs `-i claude`, and that arm needs a live `CLAUDE_CODE_OAUTH_TOKEN` in `.env` — an expired one aborts the session before the agent starts, with `Agent session is not authenticated (matched: "OAuth session expired")`, and only the user can refresh it (`claude setup-token`).
...
- **No test-fitting.** If an acceptance scenario fails, first determine whether the problem is in the skill/agent or in the scenario. Signs of test-fitting: userQuery hints at the correct approach, simulatedUser/persona scripts the exact answer, mocks leak internal logic, setup pre-creates artifacts the skill should produce. Fix the skill/agent first; adjust the scenario only if the scenario itself is wrong.
- **Do not assert that the agent READ `AGENTS.md`.** Codex (and Claude Code) load the project instructions into the system prompt before the first turn, so the rollout carries no read of that file even when the agent quotes it verbatim. A checklist item `reads_agents_md` fails on a run that did everything right. Assert the observable consequence instead — the derived value, the quoted sentence, the file written with the project's convention — and let the item say "read it OR quoted it". Observed 2026-09-04 on `tasks-overview-basic`: the agent quoted four AGENTS.md sentences and the judge failed it for not opening the file.
...
2. For acceptance-scenario behavioral failures (skill/command/agent scenario where exit code is fine but checklist score is low): READ `acceptance-tests/runs/latest/<scenario-id>/run-1/judge-evidence.md` BEFORE editing the SKILL.md. The trace shows the actual tool calls — text edits cannot fix execution paths the agent never takes (e.g., a composite skill delegating to a standalone skill via the Skill tool).
   - **Then ALWAYS open the raw agent session** — the transcript the sandboxed CLI wrote for itself. On codex (the default IDE since 2026-09-01) it is the rollout `<run>/<scenario-id>/run-1/bench-home/.codex/sessions/<YYYY>/<MM>/<DD>/rollout-<ts>-<uuid>.jsonl` (since 2026-09-15 the judge writes no rollout at all — it runs on the app-server with `ephemeral: true`, and its `CODEX_HOME` is one per run in a temp dir, outside every bench-home — so every rollout you find under a bench-home is the agent's); on claude it is `<run>/<scenario-id>/run-1/bench-home/.claude/projects/<slug>/<uuid>.jsonl`. `judge-evidence.md` is the judge's rendering; the `.jsonl` is the ground truth, and it answers questions the rendering cannot: which tools were actually invoked, with what parameters, and what the agent said to itself in between. Count the calls before theorising:
     codex: `jq -r 'select(.type=="response_item") | .payload | select(.type=="function_call" or .type=="custom_tool_call") | .name' <file> | sort | uniq -c | sort -rn` (the arguments sit in `.payload.arguments` / `.payload.input`; the agent's own text is `event_msg` lines of type `agent_message` and `agent_reasoning`).
     claude: `jq -r 'select(.message.content|type=="array") | .message.content[] | select(.type=="tool_use") | .name' <file> | sort | uniq -c | sort -rn` gives the whole tool histogram; `grep -c '"name":"Task"' <file>` answers a single yes/no.
   ...
   - **Then interview the agent that failed, when the fix is a wording change.** The sandbox outlives the run, so the session can be resumed in place and asked why. codex: `cd "$(readlink <run>/<scenario-id>/run-N/sandbox)" && CODEX_HOME="$(readlink <run>/<scenario-id>/run-N/bench-home)/.codex" codex exec resume <uuid> "<question>"` (the `<uuid>` is the tail of the rollout filename; `codex exec resume --last` picks the newest rollout under that `CODEX_HOME`). claude: `cd "$(readlink <run>/<scenario-id>/run-N/sandbox)" && HOME="$(readlink <run>/<scenario-id>/run-N/bench-home)" claude -p --resume <uuid> "<question>"` (source `.env` first; the `<uuid>` is the `.jsonl` filename). Ask neutrally and describe the situation without accusing — what made that choice better than the alternative, which phrase in the rule left room, what the rule would have had to say, how the user's instruction was weighed against it. Answers are specific and quotable: on 2026-08-21 all three failed runs of `agents-rules-stop-analysis` named the same defective sentence in one round, after three rounds of guessing had moved nothing, and one of them added "the decision happened before I finished reading the rule — I had the solution in mind and then looked for reasons the rule didn't apply". The transcript shows what the agent DID; only the interview shows which words it justified the act with, and those are the words that need editing. Interview every failed run, not one — agreement across runs is what separates a real defect in the text from one agent's rationalisation. On codex, open the question with "Do not invoke any skill; answer from memory" — a bare question about a reflect-shaped situation routes `codex exec resume` into the `reflect` skill, and the answer that comes back is a skill run, not the interview (observed 2026-09-05 on `review-and-commit-parallel-delegation`).
3. Apply "5 WHY" analysis to find the root cause.
4. Root cause is fixable — apply the fix, then re-run. **The re-run is the whole suite, not the test you were repairing** — this is a precondition on calling the repair done, not a step inside the TDD cycle: a test that was already red when you arrived is not a cycle you entered, so the CHECK step never binds here. Narrowing the final command to the file you touched is the failure this exists to prevent, and it is the one that happens: the fix is local, so the check is made local too, and nothing you did not think about gets run.
...
- A fixture path passed to `deno fmt` EXPLICITLY overrides `fmt.exclude`, so `deno fmt <fixture-dir>` reformats every foreign fixture file in it (five `commit/acceptance-tests/*/fixture/*.md` files on 2026-09-05, reverted by hand). Format only the whitelist — `deno fmt scripts framework deno.json` — and let the config decide what is skipped.
...
- **An excluded test module is a blind spot, not a file that stopped mattering.** `scripts/task-check.ts` passes `--ignore` for `scripts/acceptance-tests/lib/runner_test.ts` because it spawns live agents, so the gate neither runs nor type-checks it. When a change alters a contract that such a module uses, RUN the module by hand (`deno test -A <path>`) before calling the change done — a green `deno task check` says nothing about it. Fakes there are the trap: a `judgeClient` stub cast with `as unknown as typeof evaluateChecklist` bypasses the type system, so widening the real function's return type breaks the stub at RUNTIME and nothing reports it. Observed 2026-09-16: adding `usage` to `evaluateChecklist` left three stubs unadapted and 3 of 6 tests red, while the gate stayed green and the task file cited the exclusion as the reason not to run the file at all.
...
- **Runner:** `ubuntu-latest` — no `/bin/zsh`, no macOS `path_helper`, no `/etc/zprofile`. A test that spawns a host shell or another host binary must `stat` it first and run against whichever of the candidates is installed (fail loudly when none is), or it passes on the Mac and fails only on the runner. Observed 2026-09-05: `scripts/acceptance-tests/lib/acp/mock_bin_test.ts` spawned `/bin/zsh` unconditionally and CI went red on the first push.
