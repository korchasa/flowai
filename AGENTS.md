...
- **Runner:** `ubuntu-latest` — no `/bin/zsh`, no macOS `path_helper`, no `/etc/zprofile`. A test that spawns a host shell or another host binary must `stat` it first and run against whichever of the candidates is installed (fail loudly when none is), or it passes on the Mac and fails only on the runner. Observed 2026-09-05: `mock_bin_test.ts` spawned `/bin/zsh` unconditionally and CI went red on the first push.
