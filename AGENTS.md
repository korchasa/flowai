...
- `framework/atoms/<name>.md` → SRS FR-SKILL-COMPOSE; atomic source consumed by the generator. Excluded from `framework.tar.gz` (see `scripts/check-pack-refs.ts --leakage`).
- `framework/composites/<name>.md` → SRS FR-SKILL-COMPOSE; composite wrapper consumed by the generator. Excluded from the bundle.
