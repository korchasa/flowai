...
- **Distribution** — the plugin marketplace [korchasa/flowai-plugins](https://github.com/korchasa/flowai-plugins), a generated repo: `deno task build-plugins` renders every pack into `dist/claude-plugins/`, `validate-plugins` checks it, and the CI release job pushes it downstream on each `framework-v<version>` release (FR-PACKS). `deno task sync-plugins-local` points the local Claude Code marketplace at this checkout for dogfooding (README §Dogfooding the local plugin marketplace). The former `flowai` CLI ([korchasa/flowai-cli](https://github.com/korchasa/flowai-cli), JSR `@korchasa/flowai`) is ARCHIVED since 2026-06-05 and receives nothing; the `framework.tar.gz` + `.sha256` release assets it consumed are still built by CI (FR-DIST.BUNDLE.PIN) but have no live consumer.
- `.github/workflows/ci.yml`: CI/CD for this (framework) repo — runs `deno task check` and, on `feat:`/`fix:` commits, cuts a `chore(release)` plus a `framework-v<version>` GitHub release with the reproducible tarball, then builds the plugins and pushes them to `korchasa/flowai-plugins`.
...
- `scripts/build-plugins.ts`, `scripts/validate-plugins.ts`, `scripts/sync-plugins-local.ts` → [REF:fr:packs | FR-PACKS], [README §Installation](README.md#installation), README §Dogfooding the local plugin marketplace. The SRS clauses `FR-DIST.*`, `FR-LOOP`, `FR-ADAPT-INSTRUCTIONS` describe the archived `flowai` CLI and have no implementation in any live repo.
...
- **Variant Presentation**: The variants ARE the labeled options of the single question that asks the user to choose — each option carries its own Pros/Cons/Risks, and the cross-variant trade-offs follow the option list inside the same question. Do NOT write the variants out a second time as standalone blocks before the question; two copies drift and the reader reads both to answer once.
