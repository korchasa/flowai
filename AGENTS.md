...
- `framework/<pack>/commands/<name>/SKILL.md` → [README §Packs](README.md#packs), [FR-CMD-EXEC](documents/requirements.md#fr-cmd-exec-command-execution) / any `FR-*<NAME>*` clause, [SDS §3.1.1 Product Packs](documents/design.md#3.1.1-product-packs-framework), [SDS §3.0 Primitive Inventory](documents/design.md#3.0-primitive-inventory). **If the path is listed as a target in `framework/composites.yaml`, the SKILL.md is a gitignored generator build artefact — do NOT hand-edit and do NOT expect it in git; edit `framework/atoms/<name>.md` or `framework/composites/<name>.md` and let any consumer (`deno task check`, etc.) regenerate on next run, or manually `deno run -A scripts/generate-skill-composites.ts --write` (FR-SKILL-COMPOSE).**
- `framework/<pack>/skills/<name>/SKILL.md` → [README §Packs](README.md#packs), [FR-HOWTO](documents/requirements.md#fr-howto-automation-how-to) / any `FR-*<NAME>*` clause, [SDS §3.1.1 Product Packs](documents/design.md#3.1.1-product-packs-framework), [SDS §3.0 Primitive Inventory](documents/design.md#3.0-primitive-inventory). **Same gitignored-build-artefact rule as commands above.**
- `framework/atoms/<name>.md` → SRS FR-SKILL-COMPOSE, [SDS §3.0 Primitive Inventory](documents/design.md#3.0-primitive-inventory); atomic source consumed by the generator. Excluded from `framework.tar.gz` (see `scripts/check-pack-refs.ts --leakage`).
- `framework/composites/<name>.md` → SRS FR-SKILL-COMPOSE, [SDS §3.0 Primitive Inventory](documents/design.md#3.0-primitive-inventory); composite wrapper consumed by the generator. Excluded from the bundle.
- `framework/composites.yaml` → SRS FR-SKILL-COMPOSE, SDS §3.1.1 Composite Skill Generation subsection, [SDS §3.0 Primitive Inventory](documents/design.md#3.0-primitive-inventory). Excluded from the bundle.
- `framework/<pack>/agents/<name>.md` → [SDS §3.2 Product Agents](documents/design.md#3.2-product-agents-in-packs), [SDS §3.0 Primitive Inventory](documents/design.md#3.0-primitive-inventory), README §Agents (if public-facing)
