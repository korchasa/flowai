 ...
 * - SKILL.md: <SKILL_MAX_LINES lines, <SKILL_MAX_TOKENS tokens (chars/4 approximation)
 * - Catalog metadata (name+description): <FRONTMATTER_MAX_TOKENS tokens per agentskills.io spec
 *
 ...
 * are exempt from the token cap. Their byte count is mechanically
 ...
 * volume by re-introducing Skill-tool delegation. The line cap
 * (SKILL_MAX_LINES) and the frontmatter catalog cap (FRONTMATTER_MAX_TOKENS)
 * still apply — they cap authoring cruft rather than mechanically-required
 * content.
 */
