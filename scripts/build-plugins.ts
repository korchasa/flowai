...
const DEFAULT_CODEX_KEYWORDS = ["ai", "workflow", "framework", "flowai"];
const DEFAULT_CODEX_CATEGORY = "Productivity";
...
export interface PluginPackArtifact {
  pluginName: string;
  packName: string;
  description: string;
  version: string;
...
export async function buildPlugins(opts: BuildOptions): Promise<void> {
  ...
  const artifacts: PluginPackArtifact[] = [];
  for (const packName of opts.packs.slice().sort()) {
    ...
    const artifact = await buildPack({
      packName,
      packDir,
      outDir: opts.outDir,
      version,
  ...
  await emitClaudeMarketplace(
    opts.outDir,
    marketplaceName,
    ownerName,
    artifacts,
  );
  await emitCodexMarketplace(opts.outDir, marketplaceName, artifacts);
}

export const buildClaudePlugins = buildPlugins;

async function emitClaudeMarketplace(
  outDir: string,
  marketplaceName: string,
  ownerName: string,
  artifacts: PluginPackArtifact[],
): Promise<void> {
  const pluginEntries = artifacts.map((artifact) => {
    const entry: Record<string, unknown> = {
      name: artifact.pluginName,
      source: `./plugins/${artifact.pluginName}`,
      description: artifact.description,
      version: artifact.version,
      keywords: DEFAULT_KEYWORDS,
      category: DEFAULT_CATEGORY,
    };
    if (artifact.tags.length > 0) entry.tags = artifact.tags;
    return entry;
  });
  ...
  await ensureDir(join(outDir, ".claude-plugin"));
...
async function emitCodexMarketplace(
  ...
  const plugins = artifacts.map((artifact) => ({
    ...
    source: {
      source: "local",
      path: `./plugins/${artifact.pluginName}`,
    },
    policy: {
  ...
  await ensureDir(join(outDir, ".agents", "plugins"));
  await Deno.writeTextFile(
...
async function buildPack(
  ...
  const hasHooks = await emitHooks({
...
async function emitClaudePluginManifest(
  pluginRoot: string,
  opts: EmitManifestOpts,
): Promise<void> {
  ...
  if (opts.license) plugin.license = opts.license;
...
async function emitCodexPluginManifest(
  pluginRoot: string,
  opts: EmitManifestOpts & {
    packName: string;
    hasHooks: boolean;
  ...
  if (opts.license) plugin.license = opts.license;
  if (opts.hasHooks) plugin.hooks = "./hooks/hooks.json";
  await ensureDir(join(pluginRoot, ".codex-plugin"));
  await Deno.writeTextFile(
 ...
 */
async function emitHooks(opts: EmitHooksOpts): Promise<boolean> {
  if (!(await exists(opts.sourceDir))) return false;

  const hookDirs: string[] = [];
  for await (const e of Deno.readDir(opts.sourceDir)) {
    if (e.isDirectory) hookDirs.push(e.name);
  }
  if (hookDirs.length === 0) return false;
  hookDirs.sort();
...
if (import.meta.main) {
  ...
  // Kept in the CLI wrapper (not inside buildPlugins) so tests using a
  ...
  console.log(
    `Built Claude Code + Codex plugin marketplace at ${rel} (packs: ${
      packs.join(", ")
    }).`,
  );
}
...
function parseFlags(args: string[]): ParsedFlags {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    if (a === "--pack") flags.packs.push(args[++i]);
      ...
      console.log(
        "Usage: build-plugins.ts [--pack core] [--framework ./framework] [--out ./dist/claude-plugins] [--marketplace-name flowai-plugins]",
      );
      Deno.exit(0);
    } else {
