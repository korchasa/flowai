 ...
 * write hook from `acceptance_cache_precheck.ts`.
 */
...
} from "./acceptance_cache_precheck.ts";
import type { CachePrecheck } from "./acceptance_cache_precheck.ts";
...
> {
  ...
  const runDir = join(Deno.cwd(), "acceptance-tests", "runs", timestamp);
  await Deno.mkdir(runDir, { recursive: true });
  const tracer = new TraceLogger(runDir, "report.html");
  return { runDir, tracer };
}

/** Replace `acceptance-tests/runs/latest` symlink with the current run directory. */
export async function updateLatestSymlink(runDir: string): Promise<void> {
  const latestLink = join(Deno.cwd(), "acceptance-tests", "runs", "latest");
  try {
