 ...
 *   - Drift guard: any cross-package import from `scripts/acceptance-tests/lib/**`
...
Deno.test("cacheFilePath: produces pack-scoped JSON path", () => {
  ...
  assertEquals(
    ...
    "acceptance-tests/cache/core/my-scenario/claude.json",
  );
});
...
/**
 * FR-ACCEPT-ISOLATION — the cache key MUST include the adapter source so
 ...
 * `scripts/acceptance-tests/lib/adapters/` (the adapter directory walked by the
...
/**
 * Drift guard: if any file under scripts/acceptance-tests/lib/ gains a new
 * relative import that escapes `scripts/acceptance-tests/`, it MUST be added to
