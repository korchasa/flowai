 ...
 * Extracted from `scripts/task-acceptance-tests.ts` so the entry point stays small.
 */
...
Options:
  ...
  --lock <string>              Custom lock file name (default: acceptance-tests.lock)
...
export async function acquireBenchmarkLock(lockFile: string): Promise<void> {
  ...
  await Deno.mkdir(join(Deno.cwd(), "acceptance-tests"), { recursive: true });
