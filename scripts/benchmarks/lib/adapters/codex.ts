...
export class CodexAdapter implements AgentAdapter {
  ...
  getEnv(): Record<string, string> {
    // Unset CODEX session markers so a nested benchmark run doesn't inherit
  ...
  buildArgs(opts: {
    ...
    // caller (benchmark runner) spawns `codex` directly, so we prepend the
  ...
  async setupMocks(
    ...
    // falls through to real tool execution. The benchmark runner already
