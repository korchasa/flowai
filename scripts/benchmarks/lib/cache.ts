 ...
 * under `benchmarks/cache/<pack>/<scenario-id>/<ide>.json`. A re-run with an
 ...
 *
 * Mirrored in SRS (`FR-BENCH-CACHE`) and SDS §3.4.1.
 *
 ...
 *       "primitive:<relpath>":  fileHash(f) for f in primitive-dir, skipping benchmarks/,
 *       "pack.yaml":            fileHash(framework/<pack>/pack.yaml),
 *       "agents.template":      fileHash(framework/core/assets/AGENTS.template.md),
 *       "runner:<relpath>":     fileHash(f) for f in scripts/benchmarks/lib/** ∪ scripts/task-bench.ts,
 *       "cli:transform":        fileHash(cli/src/transform.ts),
 *       "cli:sync":             fileHash(cli/src/sync.ts),
 *       "config:full":          fileHash(benchmarks/config.json),
 *     }),
 *   }))
 *
 ...
 * ## Drift invariant
 *
 * If any file under `scripts/benchmarks/lib/` gains a new import that escapes
...
/**
 * Cross-package imports reached from `scripts/benchmarks/lib/**`.
...
export async function computeCacheKey(
  ...
  // 5. Runner code: scripts/benchmarks/lib/** + scripts/task-bench.ts.
  await hashDirectoryInto(
    join("scripts", "benchmarks", "lib"),
    "runner",
    hashInputs,
  );
  await hashFileInto(
  ...
  // 7. Full benchmarks/config.json (agent model, judge model, temperature).
  await hashFileInto(
    join("benchmarks", "config.json"),
    "config",
    hashInputs,
  );
