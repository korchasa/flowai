 ...
 *
 * The implementation is split across `scripts/acceptance-tests/lib/acceptance_*.ts`
 * modules so this entry point stays a thin orchestrator:
 *   - acceptance_cli.ts            — argv parsing, help text, lock file
 *   - acceptance_discovery.ts      — scenario discovery + filter/override
 *   - acceptance_cache_precheck.ts — pre-run cache lookup + post-run cache write
...
} from "./acceptance-tests/lib/acceptance_cli.ts";
import {
  discoverScenarios,
  selectScenarios,
} from "./acceptance-tests/lib/acceptance_discovery.ts";
import { precheckCache } from "./acceptance-tests/lib/acceptance_cache_precheck.ts";
import {
  ...
  runTasks,
  updateLatestSymlink,
} from "./acceptance-tests/lib/acceptance_runtime.ts";
import {
  finalizeRun,
  printDetailedErrors,
  printPassRates,
...
async function main() {
  ...
  const lockFile = join(Deno.cwd(), "acceptance-tests", lockName);
