...
- **Sub-FR heading & coverage caveat**: sub-FRs use `#### FR-PARENT.SUFFIX` (4 hashes — the dominant form for the 50+ sub-FRs, e.g. `FR-DIST.MARKETPLACE`, `FR-AI-CODE-REVIEW.EXISTING-SUITE`). `check-fr-coverage.ts` matches ONLY top-level `### FR-` (regex `/^###\s+FR-/`) and is NOT part of `deno task check`, so no `####` sub-FR is machine-checked by it. Sub-FR acceptance is enforced only at review time (FR Coverage Audit) or by manual anchor grep — a green `deno task check` does not imply a sub-FR was covered.
- **`check-fr-coverage.ts` is also blind to SALP markers and to the plain `**Acceptance:**` form.** Its `CODE_REF` regex matches only the pre-migration GFM-link comment (`// [FR-X](path.md#...)`), and its `BENCH_LINE` matches only the phrase `Acceptance verified by acceptance tests`. Code carrying the current `// [REF:fr:x | FR-X]` marker therefore reports `Code refs: 0`, and an FR whose acceptance is a runnable command reports `Benchmarks: (none declared)` — together they yield `Verdict: NO EVIDENCE` for a fully covered FR. Verified 2026-09-16 on the long-closed `FR-MAINT`. Read the tool's verdict as a lower bound and check the markers by grep before treating it as a gate failure.
