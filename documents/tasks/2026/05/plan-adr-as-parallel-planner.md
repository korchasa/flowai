...
# `flowai-plan-adr` as parallel planning skill

## Context

`flowai-plan-adr` was scoped narrowly: record one architectural decision in MADR format, then point the user to [`flowai-plan`](../../framework/core/skills/flowai-plan/SKILL.md) for the implementation breakdown. In practice this becomes a two-step UX where the agent emits "implementation — separate task via `/flowai-plan`", which loses the rationale-to-plan link, doubles prompt cost, and forces an unnecessary chat round-trip. The base mechanics of `flowai-plan` (variants → DoD with FR-tuples → critique → triage → index update) are reusable — the only differentiator is artifact format and lifetime (persistent MADR vs gitignored GODS task).
...
- **Replace `flowai-plan` entirely** — delete the plan skill, migrate its 22 benchmarks, route every task through ADR. Pros: one entry point. Cons: ADRs accumulate for trivial bug-fixes where no decision was made; large benchmark migration; breaks the "tasks are gitignored" property for routine work. *Rejected because* the user chose to keep both skills as parallel options.
- **(CHOSEN) Make `flowai-plan-adr` a parallel planner** — ADR-skill absorbs the full planning workflow (variants, DoD-tuples, index update, critique, triage), writes a MADR-style artifact under `documents/adr/`, auto-writes without round-trip, and gains an `implemented` status flipped by commit skills. `flowai-plan` is preserved unchanged as the parallel option for non-architectural tasks. Triggers stay distinct: ADR for "decision/architecture/persistent rationale", plan for "task breakdown without architectural choice".

## Decision

`flowai-plan-adr` becomes a planning-class skill that writes a MADR-format artifact at `documents/adr/<YYYY-MM-DD>-<slug>.md` containing Context, Alternatives (brief — 3–5 lines per alternative), Decision, Consequences, Definition of Done (with FR-Test-Evidence tuples), and Solution. Status set extends with `implemented`; [`flowai-commit`](../../framework/core/commands/flowai-commit/SKILL.md) and [`flowai-review-and-commit`](../../framework/core/commands/flowai-review-and-commit/SKILL.md) flip `accepted → implemented` on the commit that closes all referenced ADR DoD items. The skill auto-writes after the Solution step; chat output is summary-only (path + ID + DoD count). [`flowai-plan`](../../framework/core/skills/flowai-plan/SKILL.md) is preserved unchanged.
...
- ADR-skill triggers must be sharpened to avoid poaching trivial bug-fix queries that belong with `flowai-plan`. Trigger benchmarks (`trigger-pos-*`, `trigger-adj-*`, `trigger-false-*`) need rebalancing.
...
- Existing [`FR-DOC-ADR`](../requirements.md#fr-doc-adr-architecture-decision-record-skill-flowai-plan-adr) description (status set, sections) is now incomplete and must be revised. New `FR-DOC-ADR-LIFECYCLE` section captures status-flip mechanics.
...
- [x] FR-DOC-ADR: ADR skill produces MADR file with Context / Alternatives (brief) / Decision / Consequences / DoD / Solution sections; auto-writes without chat round-trip.
  - Benchmark: `flowai-plan-adr-full-planning-shape` (new) + `flowai-plan-adr-records-decision-with-alternatives` (existing — assertions adapted)
  - Evidence: `deno task bench -f flowai-plan-adr-full-planning-shape`
- [x] FR-DOC-ADR: chat output post-write is summary-only (path + ID + DoD count); no draft preview, no edit prompt.
  - Benchmark: `flowai-plan-adr-auto-writes-without-roundtrip` (new)
  - Evidence: `deno task bench -f flowai-plan-adr-auto-writes-without-roundtrip`
- [x] FR-DOC-ADR-LIFECYCLE: status set extends with `implemented`; SKILL.md rules section documents the lifecycle and the `accepted → implemented` transition trigger.
  - Test: `grep -q 'implemented' framework/core/skills/flowai-plan-adr/SKILL.md`
  - Evidence: `grep -q 'implemented' framework/core/skills/flowai-plan-adr/SKILL.md`
...
- [x] Revise existing `FR-DOC-ADR` description in SRS (planning + lifecycle scope); update [SDS §3](../design.md#3-components) catalog entry for `flowai-plan-adr` and the commit/review-and-commit sections.
...
1. **SRS edits** (`documents/requirements.md`):
   - Revise [`FR-DOC-ADR`](../requirements.md#fr-doc-adr-architecture-decision-record-skill-flowai-plan-adr) description: scope now includes planning workflow (variants/DoD/critique/index/auto-write); add new MADR sections to format spec.
...
2. **SDS edits** (`documents/design.md` §3):
   - Update component entry for `flowai-plan-adr` to describe the planning-class shape, auto-write behavior, and `implemented` lifecycle.
...
4. **`flowai-plan-adr/SKILL.md`** (`framework/core/skills/flowai-plan-adr/SKILL.md`) — full rewrite mirroring `flowai-plan` step structure:
   ...
   - Frontmatter `description:` rewritten to position skill as parallel planner; explicitly states it does NOT poach trivial-task queries (those go to `flowai-plan`). Add "Self-contained" hint? — no, this is not a composite.
...
7. **Benchmarks**:
   - `framework/core/skills/flowai-plan-adr/benchmarks/full-planning-shape/mod.ts` — new. Asserts DoD + Solution sections present in written file; alternatives section is brief (≤8 lines per entry).
   - `framework/core/skills/flowai-plan-adr/benchmarks/auto-writes-without-roundtrip/mod.ts` — new. Asserts no chat draft preview, no "approve?" prompt before write; file appears in single agent step.
   ...
   - Rebalance trigger benchmarks (`trigger-pos-*`, `trigger-adj-*`, `trigger-false-*`) so ADR-skill no longer activates on trivial bug-fix queries; verify `flowai-plan` trigger-pos still passes (no regressions).

8. **`documents/index.md`**: add `## ADR` section if absent; insert `- [ADR-0001](adr/2026-05-02-plan-adr-as-parallel-planner.md) — flowai-plan-adr as parallel planner — accepted`. Once implemented, status flips to `implemented` automatically (via #5/#6).

9. **Verification commands**:
   - `deno task check` — full pipeline (lint + test + skill validation).
   - `deno task bench -f flowai-plan-adr` — entire ADR-skill bench suite green (existing + new).
