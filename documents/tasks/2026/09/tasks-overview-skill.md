...
- `scripts/check-pack-refs.ts` (bundle-leakage validator) — if the new skill ships a generator-input asset (like `framework/atoms/*.md` / `framework/composites/*.md`) rather than a flat SKILL.md, it must be excluded from `framework.tar.gz` per this script's `TAR_EXCLUDES`; evidence: `/Users/korchasa/www/flowai/flowai/scripts/check-pack-refs.ts` lines 2-29.
  - **Retired 2026-09-18** (`2026/09/retire-flowai-cli-docs.md`): `framework.tar.gz` and `TAR_EXCLUDES` no longer exist. The leakage gate now walks the rendered marketplace tree `dist/claude-plugins`, so a generator-input asset is excluded by `scripts/build-plugins.ts` not shipping it, and `scripts/check-pack-refs.ts --leakage` is what proves that. The line numbers above point at the pre-retirement file; the rule they describe still holds in its new form.
