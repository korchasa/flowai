# Product Vision: AssistFlow

## 1. Vision Statement

To provide a robust **AssistFlow**—a set of commands, rules, and workflows—that enables control over AI agents in AI-first IDEs across all development and support domains, ensuring stable and predictable outcomes for any software project.
...
The solution is the **AssistFlow**—a structured system of persistent context (`AGENTS.md`), skills (`.cursor/skills`), and validation hooks that creates a rigid execution environment for the AI agent within any project.
...
4. **Tools for Maintaining Context Relevance**: Specialized commands (e.g., `/update-docs`, `/check`) and instructions force the agent to update documentation synchronously with code changes, providing "living" project memory.
5. **Enforced Standards**: `AGENTS.md` strictly defines code style, commit formats, and testing procedures, preventing the agent from improvising where standardization is required.
...
1. **Commands**: Located in `.cursor/skills/flow-*`. Manually triggered by the user. They enforce process boundaries.
...
2. **IDE Change Risk**: Cursor might change its context system or application logic. _Mitigation: Monitoring updates and rapid structure adaptation._
